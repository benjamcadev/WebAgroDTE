...
<head>
    ...
    <title>Registro de Ventas diarias boletas (RVD) | AgroDTE</title>
    <!-- Favicon-->
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h3>
                  REGISTRO DE VENTAS DIARIAS BOLETA (RVD)
                    <small>En este menu encontraras el registro de los envios de consumo de boletas diarios electronicas </small>
                </h3>
            </div>
            <!-- Basic Examples -->
                    ...
                    <div id="card_periodo" class="card">
                        ...
                        <div id="card_periodo_body" class="body">
                            <div class="row clearfix">
                                ...
                                <div class="col-md-2">
                                    <div class="form-line">
                                         <button onclick="busquedaRegistroBoletas('REGISTRO')" type="button" class="btn bg-purple btn-circle-lg waves-effect waves-circle waves-float">
 ...
 <!-- Select Plugin Js -->
    ...
    <script type="text/javascript">
         ...
         $("#lista_registro_envios_menu").addClass("active");
          $("#lista_registro_diarias_boleta_menu").addClass("active");
    ...
    $('[data-toggle="tooltip"]').tooltip({
        container: 'body'
            ...
            function busquedaRegistroBoletas(){
            ...
            dataType: "json",
            url: "Clases/Registros.php?funcion=busquedaRegistroBoletas",
            success:function(data){
                ...
                if (data.length == 0) {
                 ...
                 for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i]["tipo_docu"] == "39") {tipo_dte = "Boleta electronica";};
                    if (data[i]["tipo_docu"] == "41") {tipo_dte = "Boleta Exenta electronica";};
                    if (data[i]["tipo_docu"] == "61") {tipo_dte = "Nota Credito electronica";};
                                    
                        
                                    //IMPRIMIR EL MES Y CANTIDAD EN LA CARD DEL AÑO
                                var fila = '<tr>'+
                                    '<td>'+data[i]["id_envio_dte_fk"]+'</td>'+
                                    '<td>'+data[i]["fecha_inicio"]+'</td>'+
                                    '<td>'+data[i]["fecha_final"]+'</td>'+
                                    '<td>'+tipo_dte+'</td>'+
                                    '<td>'+formatter.format(data[i]["mnt_total"])+'</td>'+
                                    '<td>'+data[i]["folios_utilizados"]+'</td>'+
                                    '<td>'+data[i]["secuencia_envio"]+'</td>'+
                                    '</tr>';
